...
实际的业务中，图数据库常见吗 https://www.v2ex.com/t/1139275
- > 只见过 1 个项目实际用上了，做数据分析做社交图谱。
- > 现在 agent 用的多，用于 rag/用户 memory 场景
- > 图谱用了不少，搜推、数据挖掘（某些 ToB 的厂商就是靠这个赚钱的）都会用得到，但是图数据库真的用处不大
- > 全链路数据血缘用了
- > 没用过。。可能有些场景理论上来说是用图数据库更好，但是因为数据量不大用别的数据库也能做所以就凑活做了。。
- > 图数据库基本已经死了吧，只剩下几个厂商还在做，发展了十几年的数据库类型，竟然最近才出了统一的查询标准，对开发者来说很不友好
- > 跟着华为屁股后面用了 `janusgraph` ，做非洲国家全国的电信网通联关系，亲密度，机卡关系
- > 以前实习参与过一个给 jc 用的系统，大致功能可以通过一个手机号找到相关的所有联系人以及各种开房、车票等隐私信息，用的 `Gremlin`
- > 我们做风控还是用的到的。会用图数据库整理用户的一些关联信息之类的。但是谈不上常见，因为有性能要求很大，优化也麻烦，会尽量避免使用。
- > 多年前 用过 `OrientDb`, 不晓得现在啥样了
- > 安全相关，产品用过 `nebula graph` 和 `neo4j`
...
- > 主系统是 MySQL ，主要用它来存储货币流水数据。有一个业务是查询统计单日交易额。***但是这个表一直累计了一两年，大概有 20 亿条***。在 MySQL 里进行大数据查询时候基本用不了。要做一些复杂查询，查询周报的时候，需要一分钟才能出结果。要是查询跨月的数据，基本就直接就锁死数据库。 <br> 前端时间，研究了一下把数据同步到了 ClickHouse 。***同样的操作在 MySQL 中需要 30 多秒的查询，在 ClickHouse 里能在大概 2 秒时间内完成***，属于是保住饭碗了
- > 刚入行的时候，我所在的第一家公司，mysql 基本都要求单表查询，当时使用 PHP 来着，往往是在查完主数据之后，再查其他的附属信息，最后便利结果，把附属信息加回去。***后来去的公司，基本都是能一条 SQL 搞定的就不会多写，由于业务的复杂性增加，往往一条 SQL 需要链接 4-6 个表。直接面向 SQL 编程了属于是。再后来，公司业务转向广告投放业务，数据量大增，我跟着做了数仓，这里吐槽一下阿里的 ADB ，一不能用联合索引，二分析个语句给出的执行过程实在是太不直观了***。到现在我也不是很明白，什么，是最好的实践，只是跟着业务的规范走。团队是什么风格，就跟着写什么风格。
