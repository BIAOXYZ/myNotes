...
- Hive官方
  * [x] **`本地(Mac)`** 浏览器 + **`远端(Linux)上多个容器`** 用 Hive 官方镜像（ https://hub.docker.com/r/apache/hive ）在远端启动两个容器（也就是最基本的那种方式：一个 HiveServer，一个 Metastore）。然后在本地浏览器里访问 `http://远端ip:10002/` 即可看到面板。
