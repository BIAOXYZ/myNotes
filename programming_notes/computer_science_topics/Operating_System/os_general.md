...
按下开机键后的4.98秒 https://mp.weixin.qq.com/s/HtyJNrwpL2fQkbszM7nvAg

重启就是关机再开机？你以为错了…… https://www.ithome.com/0/651/450.htm || https://mp.weixin.qq.com/s/aLOP8bhNixlVkQ3F2QWaHQ
- 回复里的：
  * > 这文章有点浅入深出了，简单来说，重启软件系统重新载入了，但硬件不是全部关闭断电重启，因此有的与该硬件有关的故障重启不一定能解决
  * > 从嵌入式的角度说部分寄存器和内存是保留的，软复位不会清空里面的数据，必须得掉电复位才行
  * > 看到s0血压就上来了，都不知道微软想啥让各家把s3砍了搞个毛病一堆的s0睡眠
  * > 如果我没记错的话Windows8以上不关闭快速开机等设置的情况，关机并不是真正意义上的关机？
    >> 是的，现在win8之后都不是真正的关机，只是休眠了
  * > windows默认会保存很大的分页文件，约等于睡眠 c盘根目录下隐藏的pagefile.sys，还有一些bios的快速启动也会导致新设备不正常
    >> 我新装的电脑都会 去设置关闭虚拟内存 删掉休眠
