...
- 回复：
  * > 看来公安部下属的几家研究院终于阶段性分出了胜负 之前的 eid 似乎寄了
    >> 因为`三所`的 eID 有线下属性，`一所`的 CTID 纯线上。需求决定导向，所以就 CTID 路线了。其实在 2022 年底就已经阶段性分出胜负了。
    >>> 早就寄了，貌似只有航旅纵横支持eID登录，而且eID不支持iOS平台，需要内嵌手机的芯片和特殊SIM卡的支持
    >>>> 工银mudanic卡背面就写的有eid
    >>>>> eID本身是要线下代替身份证的，并且可信可控最重要可遗失，所以你看它的搭载设备，银行卡、 特制U盾、特制SIM卡，或者国内手机各种Pay，和CTID没走在一条路上。但实际上这玩意推广没带法规，所以到现在用在酒店就深圳龙岗南山两个区的，机场更是全国不到十个......
    >>>>>> 身份有三个，国家网络身份认证，ctid,eid，前面两个是一样的吗
    >>>>>>> CTID 和网络身份认证公共服务 APP 背后的技术方都是`中盾安信公司`（`公安部一所`的全资子公司）
    >>>>>>>> eid是线下电子身份证，你住酒店没带身份证可以直接用eid，国内不少酒店都支持直接使用eid了，这个是面向线上实名认证的，两个工作范围都不一样
  * > 从历史的经验看，最后大概率是一刀切层层加码，网证和身份证都得给。
