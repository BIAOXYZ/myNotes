...
为什么PG将主宰AI时代的数据库 https://mp.weixin.qq.com/s/dkQaj4xt11uHrwGK6Dm4QA  【`12.1`】
- > FDW：零胶水架构与位置透明
  * > 通过 FDW，Postgres 可以挂载万物：DuckDB、MySQL、Redis、Kafka、S3 上的 CSV，甚至是 Stripe 的 API 或系统监控指标。
  * > Supabase 甚至专门做了一个 Rust FDW 框架 wrappers，以及一个专门的网站：`fdw.dev` ，提供几十个将外部数据接入 PG的包装。
- > 接口标准化：psql 即 IDE
...
打造一个立足中国，面向世界的PG数据库发行版 https://mp.weixin.qq.com/s/kYl31hRXDvE65i_eewJ68A  【`11.29`】
- > 对扩展作者的邀请
  * > 对于扩展作者来说，`PGEXT.CLOUD` 是一个让你的作品触达全球用户的渠道。你写了一个 PostgreSQL 扩展，怎么让用户用上？自己编译打包适配十几个不同的 Linux 发行版？自己去触达全世界的 PG 用户？如果你有这样的烦恼，也许老冯可以帮到你。
- 回复：
  * > 冯老师，snowflake最近发布了`pg_lake`这款插件，也是整合了duckdb和`pg_mooncake`竞争OLAP领域头把交椅，想听您对这款插件的看法
    >> 这个就是CrunchyData原来的`bridge`，早期DuckDB缝合赛选手，实力在线，有待生态检验。扩展已经在 TODOLIST 里了，流程已经跑通，尽量下个版本加进来。
