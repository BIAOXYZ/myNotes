...
- > **为什么需要向量化执行引擎？其与动态代码生成有何关系？**
  * > **火山模型及其缺点**
    ...
    + > **类型装箱**：对于`a + 2 * b`之类表达式，由于需要对不同数据类型的变量做解释，所以在Java中需要把这些本来是primitive（如int等类型）的变量包装成Object，但执行时又需要调用具体类型的实现函数，这本质上也是虚函数调用的效率问题；
  ...
  * > **动态代码生成**
    ...
    + > 具体实现来说，***JVM系如Spark SQL，Presto可以用反射，C++系的`Impala`则使用了`llvm`生成中间码***。相对来说，C++的效率更高。
