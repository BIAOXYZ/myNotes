...
- 为什么感觉同样的软件 在 mac 上使用比 win 上使用要慢或者卡顿？ https://www.v2ex.com/t/797439
...
Mac电脑开机密码忘记如何破解 https://jingyan.baidu.com/article/f25ef254af33a3482c1b823a.html
...
- > 按住键盘上的 `Fn`（功能）键可在触控栏中看到 `F1` 到 `F12`。
- > 您还可以让功能键在您使用特定 App 时自动显示：
  * > 选取苹果菜单 `` > `“系统偏好设置”`，然后点按`“键盘”`。
  * > 点按窗口顶部的`“快捷键”`，然后从左侧列表中选择`“功能键”`。
  * > 点按“添加”按钮（`+`），然后选择一个 App 并点按“添加”。对于您添加的每个 App，在您使用这个 App 时，功能键都会自动出现在触控栏中。

mac 关闭 f11 显示桌面快捷键 https://segmentfault.com/a/1190000041963415
- > 使用 vscode 调试代码的时候, 需要用到 F11 这个键, 但是在 mac 电脑上这个快捷键被占用了.解决占用的方法:
```
系统偏好设置 - 键盘 - 快捷键 - 调度中心
```
- > 取消`显示桌面`的勾选即可
...
- > 之前看到过快捷键，`comm + tab` 是不同程序切，comm + ` 是同一程序不同窗口切换
...
MacOS中Dock栏的设置和使用技巧，新手必看 - 马可菠萝的文章 - 知乎 https://zhuanlan.zhihu.com/p/37490862
